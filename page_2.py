...
def page2():
    ...
    st.title("Prediciting Success of a Restaurant in Bangalore City Using Random Forest")
   ...
   # Add the second button and set its position to the right bottom
    if st.button("Go to Next Page", key="button_2"):
        st.write("Button 2 Clicked!")
        st.experimental_set_query_params(page="page3")

  
    # Use custom CSS to position the second button at the right bottom
    st.markdown(
        """
        <style>
            div.stButton > button#button_2,
            div.stButton > button#prev_button {
                position: fixed;
                bottom: 10px;
                right: 10px;
            }
